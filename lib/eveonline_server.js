...
var getTokenResponse = function(query) {
    ...
    var responseContent;
    try {
        responseContent = HTTP.post(config.isTest ? 'https://sisilogin.testeveonline.com/oauth/token' : 'https://login.eveonline.com/oauth/token', {params: {
...
var getVerification = function(accessToken) {
    ...
    try {
        return HTTP.get(config.isTest ? 'https://sisilogin.testeveonline.com/oauth/verify' : 'https://login.eveonline.com/oauth/verify', {
            headers: {
...
OAuth.registerService('eveonline', 2, null, function(query) {
    ...
    if (
        Meteor.users.find({
            'profile.eveOnlineCharacterId': verificaton.CharacterID
        }, {
            fields: {
                _id: 1
            }
